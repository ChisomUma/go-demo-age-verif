...
There are two things to note in the request body:

* **`resourceId`**: The journey hash and version you copied from the Journey Builder dashboard, for example `abc123...@latest`.
* **`context.config.delivery: "api"`**: This tells GBG you are consuming the journey via the API rather than a hosted web flow. The `subject` object is left empty because data is collected through interactions.
...
This route forwards the complete submission payload to the GBG `journey/interaction/submit` endpoint. The payload contains three key parts, which are assembled by the `VerificationForm` component:

* **`instanceId`** and **`interactionId`**: Identifies which journey instance and interaction this submission belongs to.
* **`participants`**: An array of domain element IDs that were collected, for example `[{ domainElementId: "FullName" }, { domainElementId: "DateOfBirth" }]`. This tells the API which elements you are providing data for.
* **`context.subject`**: The structured identity data built by `buildSubjectPayload` — names, date of birth, address, phone, email, and document data in the format the API expects.
