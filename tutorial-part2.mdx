...
The `buildSubjectPayload` function iterates over all collected domain elements, calls each element's `toSubject` mapper, and merges the results into a single `identity` object. This means each domain element only knows about its own fields, but the final payload contains data from all elements across all pages.
...
| `submitting` | Gathers all form values across all pages, builds the API payload using `buildSubjectPayload`, and calls `submitInteraction`. |
| `polling` | Polls `fetchJourneyState` every 2 seconds (up to 30 attempts) waiting for the journey to complete. |
